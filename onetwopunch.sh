...
while read ip; do
    ...
    if [[ $proto == "tcp" || $proto == "all" ]]; then 
        ...
        if [[ ! -z $ports ]]; then 
            ...
            echo -e "${BLUE}[+]${RESET} nmap -e ${iface} ${nmap_opt} -oA ${log_dir}/ndir/${log_ip}-tcp -p ${ports} ${ip}"
    ...
    if [[ $proto == "udp" || $proto == "all" ]]; then  
        ...
        if [[ ! -z $ports ]]; then
            ...
            echo -e "${BLUE}[+]${RESET} nmap -e ${iface} ${nmap_opt} -sU -oA ${log_dir}/ndir/${log_ip}-udp -p ${ports} ${ip}"
